...
var Fireworks = (function() {
  ...
  function initialize() {
    ...
    mainCanvas.style = " transform:translateZ(0);";
    mainCanvas.style.backgroundColor = "rgba(0,0,0,0);";
  ...
  function clearContext() {
    //mainContext.fillStyle = "rgba(1,1,1,0)";
   ...
   */
  function createParticle(pos, target, vel, color, usePhysics, size) {
 ...
 */
var Particle = function(pos, target, vel, marker, usePhysics, size) {
...
Particle.prototype = {
  ...
  render: function(context, fireworkCanvas) {
    ...
    context.drawImage(fireworkCanvas,
      ...
      x - 6, y - 6, 12, 12);
      */
    context.drawImage(Library.smallGlow, x - 3, y - 3, this.sizeDraw, this.sizeDraw);
...
var FireworkExplosions = {
  ...
  circle: function(firework) {
    ...
    while(count--) {
      ...
      var randomVelocity = 2 + Math.random() * 2;//4 + Math.random() * 4;
      var particleAngle = count * angle + Math.PI;
  ...
  star: function(firework) {
    ...
    var subdivisions    = 10;
    var radius          = 80;
    var randomVelocity  = -(Math.random() * 1 - 2);//-(Math.random() * 3 - 6);
